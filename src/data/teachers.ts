import type { Teacher } from '../types';
import teacherImg1 from '../assets/teachers/5287239582334263324_119.jpg';
import teacherImg2 from '../assets/teachers/5287239582334263341_119.jpg';
import teacherImg3 from '../assets/teachers/5287239582334263354_119.jpg';
import teacherImg4 from '../assets/teachers/5287239582334263356_119.jpg';
import teacherImg5 from '../assets/teachers/5287239582334263365_121.jpg';
export const sirojVideo = '/assets/sirojiddin-ustoz-video.mp4';

export const teachers: Teacher[] = [
  {
    id: '1',
    name: 'Sirojiddin Qarshiyev',
    slug: 'sirojiddin-qarshiyev-english',
    title: 'Ingliz Tili O\'qituvchisi',
    specialization: ['IELTS', 'CEFR', 'English Grammar'],
    experience: 10,
    students: 2500,
    rating: 4.9,
    bio: 'MULTILEVEL (CEFR) C1×2 sohibi, minglab o\'quvchilarning sevimli ustozi. Shogirdlari IELTS va MULTILEVEL (CEFR) dan C1 darajagacha erishgan. HIPPO, BEE xalqaro olimpiadalarining Respublika Final bosqichiga chiqqan shogirdlari bor!',
    shortBio: 'MULTILEVEL (CEFR) C1+2 sohibi, minglab o\'quvchilarni sevimli ustozi.',
    avatar: teacherImg1.src,
    coverImage: teacherImg1.src,
    certificates: [
      { id: 'c1', name: 'MULTILEVEL (CEFR) C1+2', issuer: 'CEFR', year: 2018 },
    ],
    socialLinks: {
      instagram: 'https://instagram.com',
      telegram: 'https://t.me',
    },
    introVideoUrl: sirojVideo,
    featured: true,
    languages: ['Ingliz tili', 'O\'zbek tili'],
    achievements: ['HIPO xalqaro olimpiadasi Respublika Finali', 'BEE xalqaro olimpiadasi Respublika Finali'],
  },
  {
    id: '2',
    name: 'Sirojiddin Qarshiyev',
    slug: 'sirojiddin-qarshiyev-history',
    title: 'Tarix O\'qituvchisi',
    specialization: ['Milliy Sertifikat', 'Attestatsiya', 'DTM'],
    experience: 10,
    students: 2200,
    rating: 4.8,
    bio: 'MILLIY SERTIFIKAT A+ sohibi, minglab o\'quvchilarning sevimli ustozi. Shogirdlari MILLIY SERTIFIKAT dan A va A+ darajagacha erishgan!',
    shortBio: 'MILLIY sertifikat A+ sohibi, minglab o\'quvchilarning sevimli ustozi.',
    avatar: teacherImg2.src,
    coverImage: teacherImg2.src,
    certificates: [
      { id: 'c1', name: 'Milliy Sertifikat A+', issuer: 'O\'zbekiston Respublikasi', year: 2016 },
    ],
    socialLinks: {
      instagram: 'https://instagram.com',
      telegram: 'https://t.me',
    },
    introVideoUrl: '',
    featured: true,
    languages: ['O\'zbek tili', 'Rus tili'],
    achievements: ['Milliy Sertifikat A+', 'Attestatsiya A+'],
  },
  {
    id: '3',
    name: 'Sirojiddin Qarshiyev',
    slug: 'sirojiddin-qarshiyev-literature',
    title: 'Ona Tili va Adabiyot O\'qituvchisi',
    specialization: ['Milliy Sertifikat', 'Attestatsiya', 'DTM'],
    experience: 10,
    students: 2100,
    rating: 4.8,
    bio: 'MILLIY SERTIFIKAT A+ sohibi, minglab o\'quvchilarning sevimli ustozi. Shogirdlari MILLIY SERTIFIKAT dan A va A+ darajagacha erishgan!',
    shortBio: 'MILLIY sertifikat A+ sohibi, minglab o\'quvchilarning sevimli ustozi.',
    avatar: teacherImg3.src,
    coverImage: teacherImg3.src,
    certificates: [
      { id: 'c1', name: 'Milliy Sertifikat A+', issuer: 'O\'zbekiston Respublikasi', year: 2016 },
    ],
    socialLinks: {
      instagram: 'https://instagram.com',
      telegram: 'https://t.me',
    },
    introVideoUrl: '',
    featured: true,
    languages: ['O\'zbek tili', 'Rus tili'],
    achievements: ['Milliy Sertifikat A+', 'Attestatsiya A+'],
  },
  {
    id: '4',
    name: 'Nurbek Qarshiyev',
    slug: 'nurbek-qarshiyev-english',
    title: 'Ingliz Tili O\'qituvchisi',
    specialization: ['CEFR', 'English Grammar', 'B2'],
    experience: 2,
    students: 850,
    rating: 4.7,
    bio: 'O\'zbekiston Davlat Jahon Tillari Universiteti bitiruvchisi. Shogirdlari MULTILEVEL (CEFR) dan B2 darajagacha erishgan, yuzlab mamnun o\'quvchilarning sevimli ustozi!',
    shortBio: 'O\'zbekiston Davlat Jahon Tillari Universiteti bitiruvchisi.',
    avatar: teacherImg4.src,
    coverImage: teacherImg4.src,
    certificates: [
      { id: 'c1', name: 'Ingliz Tili Bakalavr', issuer: 'O\'zbekiston Davlat Jahon Tillari Universiteti', year: 2022 },
    ],
    socialLinks: {
      instagram: 'https://instagram.com',
      telegram: 'https://t.me',
    },
    introVideoUrl: '',
    featured: true,
    languages: ['Ingliz tili', 'O\'zbek tili'],
    achievements: ['MULTILEVEL (CEFR) B2', 'Yuzlab mamnun o\'quvchilar'],
  },
  {
    id: '5',
    name: 'Munira Qarshiyeva',
    slug: 'munira-qarshiyeva-english',
    title: 'Ingliz Tili O\'qituvchisi',
    specialization: ['CEFR', 'English Grammar'],
    experience: 3,
    students: 950,
    rating: 4.8,
    bio: 'Qarshi Davlat Universiteti bitiruvchisi. Yuzlab mamnun o\'quvchilarning sevimli ustozi, shogirdlari nufuzli xalqaro olimpiadalarda ishtirok etib kelmoqda!',
    shortBio: 'Qarshi Davlat Universiteti bitiruvchisi. Yuzlab mamnun o\'quvchilarning sevimli ustozi.',
    avatar: teacherImg5.src,
    coverImage: teacherImg5.src,
    certificates: [
      { id: 'c1', name: 'Ingliz Tili Bakalavr', issuer: 'Qarshi Davlat Universiteti', year: 2021 },
    ],
    socialLinks: {
      instagram: 'https://instagram.com',
      telegram: 'https://t.me',
    },
    introVideoUrl: '',
    featured: true,
    languages: ['Ingliz tili', 'O\'zbek tili'],
    achievements: ['Xalqaro olimpiada ishtirokchisi', 'Yuzlab mamnun o\'quvchilar'],
  },
];

